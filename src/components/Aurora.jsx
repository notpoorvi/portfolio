...
void main() {
  ...
  // Clamp intensity to prevent negative values that cause black lines
  float intensity = max(0.0, 0.4 + 0.3 * height);
  
  float midPoint = 0.15;
  float auroraAlpha = smoothstep(midPoint - uBlend * 0.3, midPoint + uBlend * 0.7, intensity);
  
  // Ensure colors never go completely black
  vec3 auroraColor = mix(rampColor * 0.1, rampColor, intensity);
